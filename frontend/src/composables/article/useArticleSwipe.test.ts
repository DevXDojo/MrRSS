...
function fixture() {
  ...
  const wrapper = mount({
    setup() {
      return { swipe: useArticleSwipe({ articleId: () => id.value, enabled: () => true, previous, next }) };
    },
    template: '<div @touchstart="swipe.touchstart" @touchmove="swipe.touchmove" @touchend="swipe.touchend" @touchcancel="swipe.touchcancel" @wheel="swipe.wheel"><p>Text</p><button>Control</button><pre>Code</pre></div>',
  });
...
describe('article swipe', () => {
  it('navigates in both directions only after a deliberate horizontal swipe', () => {
    const f = fixture();
    f.touch('touchstart', 200); f.touch('touchend', 100);
    f.touch('touchstart', 100); f.touch('touchend', 200);
    f.touch('touchstart', 100); f.touch('touchend', 125);
  ...
  it('ignores vertical scrolling, controls, code, multitouch and cancelled/stale gestures', () => {
    ...
    for (const target of ['button', 'pre']) {
      f.touch('touchstart', 200, 0, target); f.touch('touchend', 0, 0, target);
    }
    f.touch('touchstart', 200); f.touch('touchmove', 195, 50); f.touch('touchend', 0);
    f.touch('touchstart', 200, 0, 'p', 2); f.touch('touchend', 0);
    f.touch('touchstart', 200); f.touch('touchcancel', 100); f.touch('touchend', 0);
    f.touch('touchstart', 200); f.id.value = 2; f.touch('touchend', 0);
  ...
  it('ignores selection and horizontal scrollers, and consumes trackpad inertia', () => {
    ...
    f.touch('touchstart', 200); f.touch('touchend', 0);
    expect(f.next).not.toHaveBeenCalled();
    selection.mockRestore();
    const paragraph = f.wrapper.get('p').element;
    Object.defineProperties(paragraph, { scrollWidth: { value: 800, configurable: true }, clientWidth: { value: 200 } });
    paragraph.style.overflowX = 'auto';
    f.touch('touchstart', 200); f.touch('touchend', 0);
    expect(f.next).not.toHaveBeenCalled();
    Object.defineProperty(paragraph, 'scrollWidth', { value: 200 });
    for (let i = 0; i < 8; i++) paragraph.dispatchEvent(new WheelEvent('wheel', { bubbles: true, deltaX: 40 }));
    expect(f.next).toHaveBeenCalledTimes(1);
    f.wrapper.unmount();
  });
});
