...
const zh: TranslationMessages = {
  ...
  modal: {
    ...
    feed: {
      ...
      picker: {
        title: '可视化 XPath 选择器',
        hint: '点击文章容器，必要时选择父级，再确认该元素。随后在同一篇文章内选择各字段，检查匹配示例后应用。',
        staticHint: '安全静态预览会省略原站样式、脚本和外部资源。仅由 JavaScript 生成的列表请使用自定义脚本；应用后请在表单中设置日期格式。',
        item: '文章容器', titleField: '标题',
        uri: '链接', timestamp: '日期', content: '摘要', thumbnail: '缩略图',
        selectElement: '请点击预览中的元素。', parent: '选择父级', choose: '选择该元素',
        withinItem: '请在已选文章内部选择字段。链接请选择链接文字，缩略图请选择图片占位块。',
        matches: '匹配 {count} 个容器', apply: '应用 XPath', retry: '重新加载预览',
        loadFailed: '无法加载网页，请检查网址、代理和网络，或改用自定义脚本。',
      },
