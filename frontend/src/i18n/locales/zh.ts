...
const zh: TranslationMessages = {
  ...
  modal: {
    ...
    rule: {
      ...
      addAction: '添加操作',
      addCondition: '添加条件',
      exportRules: '导出规则', importRules: '导入规则',
      importConfirm: '将 {count} 条规则追加至现有列表？会保留启用状态，并在后续刷新时生效；此次不立即处理已有文章。',
      importSuccess: '已导入 {count} 条规则', invalidBackup: 'MrRSS 规则备份格式无效或不受支持，未导入任何规则。',
      backupTooLarge: '规则备份超过 5 MB 限制。', importFailed: '无法导入规则，请重新加载设置后再试。',
      addRule: '添加规则',
      condition: '条件',
