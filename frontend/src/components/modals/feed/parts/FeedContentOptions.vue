...
async function save() {
  ...
  try {
    const res = await fetch(`/api/feeds/content-options?id=${props.feedId}`, {
      ...
      body: JSON.stringify({ content_selector: contentSelector.value, remove_selector: removeSelector.value, cookie_origin: cookieOrigin.value, ...(clearCookie.value ? {cookie: ''} : cookie.value ? {cookie: cookie.value} : {}) }),
    });
    if (!res.ok) throw new Error('Content options');
    const saved = await res.json();
    hasCookie.value = saved.has_cookie === true;
    cookie.value = ''; clearCookie.value = false;
...
<template>
  <fieldset class="space-y-3 border-t border-border pt-4" :disabled="loading || saving">
    ...
    <label class="block text-sm text-text-primary">
      ...
      <input v-model="cookieOrigin" data-testid="cookie-origin" class="w-full mt-1 px-3 py-2 bg-bg-secondary border border-border rounded-lg" placeholder="https://example.com" />
    </label>
    <label class="block text-sm text-text-primary">
      {{ t('modal.feed.cookie') }}
      <input v-model="cookie" data-testid="feed-cookie" type="password" autocomplete="new-password" maxlength="8192" class="w-full mt-1 px-3 py-2 bg-bg-secondary border border-border rounded-lg" :placeholder="hasCookie ? t('modal.feed.cookieSaved') : 'name=value; name2=value2'" />
    </label>
    <p class="text-xs text-text-secondary">{{ t('modal.feed.cookieHelp') }}</p>
    <label v-if="hasCookie" class="flex items-center gap-2 text-sm"><input v-model="clearCookie" type="checkbox" />{{ t('modal.feed.clearCookie') }}</label>
