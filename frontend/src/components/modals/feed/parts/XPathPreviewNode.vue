...
<template>
  ...
  <component
    ...
    v-else
    class="picker-node"
    :class="{ 'picker-selected': node.path === selected, 'picker-image': node.tag === 'img' }"
    @click.stop="emit('pick', node)"
  >
...
p,
blockquote {
  @apply my-2 p-2 border border-border rounded;
}
h1,
h2,
h3,
...
.picker-image {
  @apply inline-block p-2 bg-bg-tertiary text-text-secondary rounded;
}
</style>
