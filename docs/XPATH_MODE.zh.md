...
可视化选择器支持撤销／重做（预览内 Ctrl/Cmd+Z、Shift+Ctrl/Cmd+Z）、简化页面布局、
父子元素导航（Alt+上／下）与 Enter 确认。提取结果分页显示，可只看缺少标题或链接的
结果，并定位到页面对应位置。只有在全部选中文章中一致匹配到唯一日期或图片时才自动
建议这些字段。应用可视化结果会保留手动配置的作者、分类表达式和自定义 UID。
