...
先点击文章，使用“选择父级”定位完整的重复文章容器，再确认元素。随后在同一篇文章
内部选择标题、链接，以及可选的日期、摘要、缩略图。检查匹配数量和示例后应用，
日期格式仍在表单中设置。应用会替换 XPath 配置，提交订阅表单前不会保存。

预览保留网页结构，使用简化样式和图片占位块，不运行目标网页脚本、不加载外部资源、
不使用浏览器登录状态，也不能点击链接跳转。预览和添加订阅均使用表单中的代理配置。
生成的路径依据当前网页结构，站点调整结构后可能需要重新选择或手动修改。
