...
func (h *Handler) FetchFullArticleContentContext(ctx context.Context, articleURL string, source *models.Feed) (string, error) {
	result, err := h.FetchFullArticleForReader(ctx, articleURL, source)
	// Existing content consumers/cache retain the unfiltered sanitized source.
...
func (h *Handler) FetchFullArticleForReader(ctx context.Context, articleURL string, source *models.Feed) (adfilter.Content, error) {
	parsedURL, err := url.Parse(articleURL)
	if err != nil || parsedURL.Host == "" || (parsedURL.Scheme != "http" && parsedURL.Scheme != "https") {
		return adfilter.Content{}, fmt.Errorf("invalid article URL")
	}
	client, err := h.createArticleHTTPClient(source)
	if err != nil {
		return adfilter.Content{}, err
	}
	options := database.FeedContentOptions{}
	if source != nil {
		options, err = h.DB.GetFeedContentOptions(ctx, source.ID)
		if err != nil {
			return adfilter.Content{}, fmt.Errorf("read content options: %w", err)
		}
	}
	...
	if err != nil {
		return adfilter.Content{}, fmt.Errorf("create request: %w", err)
	}
	req.Header.Set("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36")
	req.Header.Set("Accept", "text/html,application/xhtml+xml;q=0.9,*/*;q=0.8")
	resp, err := client.Do(req)
	if err != nil {
		return adfilter.Content{}, fmt.Errorf("fetch page: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return adfilter.Content{}, fmt.Errorf("fetch page: HTTP %d", resp.StatusCode)
	}
	reader, err := charset.NewReader(io.LimitReader(resp.Body, maxArticleBytes+1), resp.Header.Get("Content-Type"))
	if err != nil {
		return adfilter.Content{}, fmt.Errorf("decode page: %w", err)
	}
	body, err := io.ReadAll(io.LimitReader(reader, maxArticleBytes+1))
	if err != nil {
		return adfilter.Content{}, fmt.Errorf("read page: %w", err)
	}
	if len(body) > maxArticleBytes {
		return adfilter.Content{}, fmt.Errorf("article exceeds 10 MB")
	}
	doc, err := goquery.NewDocumentFromReader(bytes.NewReader(body))
	if err != nil {
		return adfilter.Content{}, fmt.Errorf("parse page: %w", err)
	}
	...
	if strings.TrimSpace(options.ContentSelector) != "" {
		...
		if selected.Length() == 0 {
			return adfilter.Content{}, fmt.Errorf("content selector matched no elements")
		}
		...
		content := h.PrepareArticleForReader(ctx, fragments.String(), base.String())
		if content.Content == "" && content.OriginalContent == "" {
			return adfilter.Content{}, fmt.Errorf("content selector produced empty content")
		}
		return content, nil
	}
	...
	page, err := doc.Html()
	if err != nil {
		return adfilter.Content{}, fmt.Errorf("render page: %w", err)
	}
	...
	prepared := h.PrepareArticleForReader(ctx, content, base.String())
	if strings.TrimSpace(prepared.Content) == "" && strings.TrimSpace(prepared.OriginalContent) == "" {
		return adfilter.Content{}, fmt.Errorf("no readable article content")
	}
	return prepared, nil
}
